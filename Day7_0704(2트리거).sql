...
BEFORE UPDATE OR DELETE ON tbl_custom		-- 트리거가 동작하는 테이블 ,SQL과 시점
BEGIN 
...
FOR EACH ROW 	-- 만족(적용)하는 행이 여러개일때, :OLD UPDATE 또는 DELETE 하기전 값, :NEW는 INSERT 한 값
BEGIN 
	-- 구매취소(tbl_buy 테이블에서 삭제)한 
	-- 데이터 tbl_temp 임시 테이블에 INSERT : 여러행에 대한 작업(행 트리거)
...
--	ㄴ grant create view to c##idev; -> 뷰 생성 권한 없는 오류 생기면 추가 권한 부여하세요
CREATE VIEW v_buy
AS
